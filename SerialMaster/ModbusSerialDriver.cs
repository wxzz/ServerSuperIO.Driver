...
{
    public class ModbusSerialDriver : ServerSuperIO.Device.RunDevice
    {
        /// <summary>
        /// 这是Server框架必须使用的对象
        /// </summary>
        private ModbusSerialProtocol _rtuPro;
        private IModbusMaster _modbusRtuMaster;
        private IModbusMaster _modbusAsciiMaster;
        ...
        {
            _rtuPro = new ModbusSerialProtocol();
            _modbusRtuMaster = ModbusMasterFactory.CreateRtu();
            _modbusAsciiMaster = ModbusMasterFactory.CreateAscii();
        }
        ...
        public override IList<IRequestInfo> GetConstantCommand()
        {
            ...
            foreach(ITag tag in tags)
            {
                ...
                if(tag.Function == Modbus.Modbus.ReadCoils)
                {
                    ...
                    if (tag.Mode == "RTU")
                    {
                        reqeustBytes = _modbusRtuMaster.BuildReadCoilsCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    else if(tag.Mode== "ASCII")
                    {
                        reqeustBytes = _modbusAsciiMaster.BuildReadCoilsCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    #endregion
                }
                else if(tag.Function ==Modbus.Modbus.ReadInputs)
                {
                    ...
                    if (tag.Mode == "RTU")
                    {
                        reqeustBytes = _modbusRtuMaster.BuildReadInputsCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    else if (tag.Mode == "ASCII")
                    {
                        reqeustBytes = _modbusAsciiMaster.BuildReadInputsCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    #endregion
                }
                else if(tag.Function == Modbus.Modbus.ReadHoldingRegisters)
                {
                    ...
                    if (tag.Mode == "RTU")
                    {
                        reqeustBytes = _modbusRtuMaster.BuildReadHoldingRegistersCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    else if (tag.Mode == "ASCII")
                    {
                        reqeustBytes = _modbusAsciiMaster.BuildReadHoldingRegistersCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    #endregion
                }
                else if(tag.Function == Modbus.Modbus.ReadInputRegisters)
                {
                    ...
                    if (tag.Mode == "RTU")
                    {
                        reqeustBytes = _modbusRtuMaster.BuildReadInputRegistersCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    else if (tag.Mode == "ASCII")
                    {
                        reqeustBytes = _modbusAsciiMaster.BuildReadInputRegistersCommand((byte)tag.SlaveId, (ushort)tag.Address, (ushort)tag.Quantity, out requestMsg);
                    }
                    #endregion
                }
